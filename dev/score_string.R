...
#convert.score <- function(arg.score.string) {
  ...
  arg.score.string <- "7-6(5) 6-7(8) 6-1 4-2 40-15"
  completed <- FALSE
  ...
  if (num.completed.sets > 0) {
    for (set.num in 1:num.completed.sets) {
      ...
      p1.total.tiebreaks <- p1.total.tiebreaks + ifelse(get.set.winner(scores[set.num])[[1]]==7,1,0)
      p2.total.tiebreaks <- p2.total.tiebreaks + ifelse(get.set.winner(scores[set.num])[[2]]==7,1,0)
    }
  }
