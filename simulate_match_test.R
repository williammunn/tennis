...
play.match <- function(arg.best.of,arg.match.first.server,arg.match.p1.serve.pct,arg.match.p2.serve.pct,arg.match.inherited.score,arg.match.inherited.server) {
  ...
  inherit.set <- FALSE
  # inherited
  if (hasArg(arg.match.inherited.score)) {
...
play.tiebreak <- function(arg.tiebreak.first.server,arg.tiebreak.p1.serve.pct,arg.tiebreak.p2.serve.pct,arg.tiebreak.inherit,arg.tiebreak.inherited.score) {
  # initial values
  tiebreak.over <- FALSE
  ...
  if (arg.tiebreak.inherit) {
    ...
    p1.points <- inherited.score[["p1.points"]]
    p2.points <- inherited.score[["p2.points"]]
    tiebreak.current.server <- arg.tiebreak.inherited.server
  }
